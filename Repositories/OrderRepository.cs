...
namespace cityshop_api.Repositories
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task<string> CreateOrder(OrderRequest orderRequest)
        {
            ...
            if (orderRequest.OrderBy == null)
                throw new ArgumentException("Customer details are required");
            ...
            customerTable.Columns.Add("CustomerPhone", typeof(string));
            customerTable.Columns.Add("CustomerEmail", typeof(string));
            customerTable.Columns.Add("CustomerAddress", typeof(string));
            customerTable.Columns.Add("Pincode", typeof(string));

            customerTable.Rows.Add(
                orderRequest.OrderBy.CustomerName,
                orderRequest.OrderBy.CustomerPhone,
                orderRequest.OrderBy.CustomerEmail,
                orderRequest.OrderBy.CustomerAddress,
                orderRequest.OrderBy.Pincode
            );

            // Items TVP
            var orderItemTable = new DataTable();
            orderItemTable.Columns.Add("ItemId", typeof(Guid));
            orderItemTable.Columns.Add("SizeId", typeof(Guid));
            orderItemTable.Columns.Add("ItemQuantity", typeof(int));

            foreach (var item in orderRequest.OrderedItems)
            {
                orderItemTable.Rows.Add(item.ItemId, item.SizeId, item.Qty);
            }
            ...
            // ShopId
            command.Parameters.Add(new SqlParameter("@ShopId", orderRequest.ShopId)
            {
                SqlDbType = SqlDbType.UniqueIdentifier
            });

            // Customer TVP
            command.Parameters.Add(new SqlParameter("@Customer", customerTable)
            {
            ...
            // Items TVP
            command.Parameters.Add(new SqlParameter("@Items", orderItemTable)
            {
                SqlDbType = SqlDbType.Structured,
            ...
            // OUTPUT param
            var outputParam = new SqlParameter("@OrderNumber", SqlDbType.VarChar, 50)
            {
            ...
            string orderNumber = outputParam.Value?.ToString();
            return ($"Order created with order number {orderNumber}");
        }
